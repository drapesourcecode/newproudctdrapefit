...
<div class="live-chat" id="btn1" href="javascript:void(0)">
    <div class="live-chat-top" onclick="togglemargin()">
        What can we help with? 
        <span> 
            <a> <i class="fas fa-minus"></i> </a> 
            <a id="btnClose"> <i class="fas fa-times"></i> </a> 
        </span>
    </div>
...
<script type="text/javascript">
    //var dd='ws://108.167.139.168:8081';
    var conn;
    function init() {
...
//            alert(data);
            ...
            if (data.type == "chat") {
                ...
                if (userId == data.userId) {
                    ...
                    var row = '<tr><td valign="top" style="widht:100%;"><div class="send-go" style="float: right;text-align: left;width: 90%;font-size:12px;color: #fff;background: #d64ade;padding: 5px 10px;border-radius: 4px;position: relative;">' + data.msg + '</div><span style="float: right;text-align: right;width: 90%;font-size:9px !important;">' + data.timeChat + '</span></td>\n\</tr>';
                } else {
            ...
            success: function (data) {
                ...
                for (i = 0; i <= data.msg.length; i++) {
                    ...
                    if (userId == data.msg[i].user_id) {
                        var row = '<tr><td valign="top" style="widht:100%;"><div class="send-go" style="float: right;text-align: left;width: 90%;font-size: 13px;color: #fff;background: #d64ade;padding: 7px 12px;border-radius: 4px;position: relative;">' + data.msg[i].chat_message + '</div><span style="font-size:9px !important;float: left;text-align: right;width: 90%;">' + data.time[i] + '</span></td>\n\</tr>';
                    } else {
